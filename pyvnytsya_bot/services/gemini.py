...
class AIService:
    ...
    async def generate_ending(self, survivors_info: str, scenario: str, custom_prompt: str = None) -> str:
        if custom_prompt:
            # Inject dynamic variables into custom prompt if user used placeholders (optional but good practice)
            # For now, let's assume the user prompt is the instruction, and we append the context.
            # Or better: The user prompt replaces the INSTRUCTION part, but we still need to provide the context (scenario + survivors).
